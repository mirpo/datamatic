...
func (p *PromptStep) generateWithRetry(ctx context.Context, cfg *config.Config, provider llm.Provider, req llm.GenerateRequest) (*llm.GenerateResponse, error) {
	if !cfg.RetryConfig.Enabled {
		return provider.Generate(ctx, req)
	}

	return retry.Execute(ctx, cfg.RetryConfig.ToRetryConfig(), func() (*llm.GenerateResponse, error) {
		return provider.Generate(ctx, req)
	})
}
...
func (p *PromptStep) Run(ctx context.Context, cfg *config.Config, step config.Step, outputFolder string) error {
	...
	for i < maxResult {
		...
		response, err := p.generateWithRetry(ctx, cfg, provider, llm.GenerateRequest{
